...
class ConfluentConsumerThread(ConsumerThread):
    ...
    def _create_admin(self, transport: "Transport"):
        return AdminClient(
    ...
    async def create_topic(
        ...
        *,
        ...
        admin = self._create_admin(transport)
        topic = NewTopic(topic, partitions, replication, **config)
        await self.call_thread(self._create_topic, admin, topic)

    async def _create_topic(self, admin_client: AdminClient, topic: NewTopic):
        topics_created = admin_client.create_topics([topic])
        topics_created[topic.topic].result()
